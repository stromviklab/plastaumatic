...
if [ ! -f $seed ] || [ ! -f $ref_fasta ] || [ ! -d $ref_gb ] || [ ! -d $out ] || [ ! -f $path_to_trimmomatic ] || [ ! -f $adapters ] || [ ! -f $path_to_novoplasty ] || [ ! -f $path_to_PGA ] || [ ! -d $repo ] || [ ! -f $fof ];then 
    echo -e "ERROR:\t Input files does not exist, check again" 
    exit 1
fi 

# Set memory and cpus available 
threads_available=$(nproc --all)
...
# #check for ISC
# ${repo}/check_internal_stops.sh ${WORKDIR}/annotation/out_plastome/${prefix}.plastome.mod.gb ${WORKDIR}/annotation/target_plstome/${prefix}.plastome.mod.fa
