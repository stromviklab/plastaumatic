...
if [ -f $1 ] && [ -f $2 ];then 
    ...
    id=$(cut -f1 $2.fai)
    echo -e "Below are the gene features with internal stop codons"
    grep -E -v "join|complement" _list |grep -v -wF "rps12"|while read i 
    do
        name=$(echo $i|awk '{print $1}')
        loc=$(echo $i|awk '{print $2}'|awk -F '.' '{print "'$id':"$1"-"$3}')
        count=$(samtools faidx  $2 $loc|grep -v ">" |perl translateDna -i -|grep -c '*')
        if [ $count -gt 1 ];then 
    ...
    do 
        ...
        count=$(samtools faidx -i $2 `echo $loc` |perl translateDna -i -|grep -c "*")
        if [ $count -gt 1 ];then 
            ft=$(echo $i|awk '{print $2}')
            echo -e "$name:$ft gene feature has internal stop codons"
        fi 
    ...
    if [ $(grep -c -wF "rps12" _list) -eq 3 ];then 
        ...
        count=$(echo $c1fa $j2_3 |tr -d ' ' |perl translateDna -i -|grep -c "*")
        if [ $count -gt 1 ];then 
